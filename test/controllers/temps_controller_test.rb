...
class TempsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create temp" do
    assert_difference("Temp.count") do
      post temps_url, params: { temp: { email: @temp.email, login: @temp.login, name: @temp.name } }
    end
  ...
  test "should update temp" do
    patch temp_url(@temp), params: { temp: { email: @temp.email, login: @temp.login, name: @temp.name } }
